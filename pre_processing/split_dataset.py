...
def split_dataset(dataset_path, train_size=0.7,validation_size=0.8):
    ...
    for dir in os.listdir(dataset_path):
        ...
        for file in os.listdir(dataset_path + "/" + dir):
            ...
            if not os.path.exists("data/split/mfcc/train/" + dir):
                # Create the directory if it doesn't exist
                os.makedirs("data/split/mfcc/train/" + dir)
            if not os.path.exists("data/split/mfcc/validation/" + dir):
                # Create the directory if it doesn't exist
                os.makedirs("data/split/mfcc/validation/" + dir)
            if not os.path.exists("data/split/mfcc/test/" + dir):
                # Create the directory if it doesn't exist
                os.makedirs("data/split/mfcc/test/" + dir)
            
             # Split the dataset in training and testing
            if i < train_size*100:
                # Move the file to the folder "train"
                os.rename(file_path, os.path.abspath("data/split/mfcc/train/" + dir + "/" + file))
            elif( train_size*100 <= i < validation_size*100):
                #move the file to the folder "validation"
                os.rename(file_path, os.path.abspath("data/split/mfcc/validation/" + dir + "/" + file))
            else : 
                # Move the file to the folder "test"
                os.rename(file_path, os.path.abspath("data/split/mfcc/test/" + dir + "/" + file))
            i+=1

def main():
    # Parse the arguments
    parser = argparse.ArgumentParser(description="Split the dataset into training and testing dataset")
    parser.add_argument("--dataset_path", type=str, default="mfcc", help="Path to the dataset")
